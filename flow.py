...
"""
@brief: spatial analysis of flow for the tangible topography experiment
...
def main():
    ...
    for dem in dems:
        ...
        before=region.replace("dem","depth")
        after=depth
    ...
    for i in range(1,8):
        ...
        reference_depth = "depth_"+region[-1:]+"@PERMANENT"
        ...
        # compute sum of differences of depths
        gscript.run_command('r.series', input=diff_list, output=sum_diff, method="sum", overwrite=overwrite)
        gscript.run_command('r.colors', map=sum_diff, color="differences")
        gscript.run_command('d.mon', start=driver, width=width, height=height, output=os.path.join(series,sum_diff+".png"), overwrite=overwrite)
        gscript.run_command('d.shade', shade=reference_relief, color=sum_diff, brighten=75)
        gscript.run_command('d.vect', map=reference_contour, display='shape')
        gscript.run_command('d.legend', raster=sum_diff, fontsize=10, at=(10,90,1,4))
        gscript.run_command('d.mon', stop=driver)
