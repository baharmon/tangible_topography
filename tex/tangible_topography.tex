...
that gives 3D spatial data an interactive, physical form so that users can naturally feel it, see it, and shape it.
%
Tangible Landscape couples a physical and a digital model of a landscape
through a near real-time cycle of 
physical manipulation, spatial computation, and projected feedback.
%
As users manipulate % interact with 
...
%
Our aim was to iteratively design and empirically test a tangible interface that augments 3D spatial thinking and improves 3D spatial performance. 
%
...
%
We designed Tangible Landscape to physically manifest 
3D data such as topography 
% topographic data / geographic data
so that users can cognitively grasp that data %the terrain %
...
% REFRAME in terms of 
% multi-dimensional spatial thinking
...
%
GIS, however, can be unintuitive and challenging to use. % cite?
%
...
%
Since visually thinking about 3D space requires %draws on 
...
%
Through a series of experiments we have begun to
...
%
3D spatial thinking for example is used in disciplines 
like geology to understand the structure of the earth, 
...
form the basis of geographic knowledge \cite{Golledge1997,Golledge2002,Golledge2016}. 
%
...
%
Given the importance of spatial thinking 
...
%
In scientific computing multidimensional spatial patterns and processes can be 
...
% RE-FOCUS ON TANGIBLE GEOSPATIAL MODELING
% tangible interaction with surfaces
% lit review
% what we know -> studies done
...
\subsection{Tangible geospatial modeling} % Tangible geospatial interaction
% tangible interfaces for geospatial modeling

% MORE INTRO...

Tangible interfaces for geospatial modeling include augmented architectural models (Table~\ref{table:review_a}), augmented clay (Table~\ref{table:review_b}),  augmented sandboxes (Table~\ref{table:review_c}), and actuated pin tables (Table~\ref{table:review_d}).
% -- a type of shape-changing interface. 
%
% augmented 
...
%Given the need to have working prototype to study
%research on tangible interfaces for geospatial modeling 
%has understandably focused on the design of new technologies and prototypes 
%
Research on tangible interfaces for geospatial modeling 
has focused on the design of new technologies and prototypes
rather than studying how they % tangibles 
are used. 
%
% 
%
...
shows that have been relatively few qualitative user studies \cite{Ishii2002,Shamonsky2003,Tateosian2010,Petrasova2015}
and only a single quantitive pilot study \cite{Harmon2016}.
Most of the qualitative studies were case studies, 
which lacked rigorous, systematic qualitative analysis.
%
...
in order to understand if the interfaces work as designed
\cite{Rasmussen2012}.
%
...
% add reference to immersive tangible geospatial modeling
...
\begin{center}
	...
	\caption{Landforms identified by \textit{r.geomorphon}\footnote{\url{https://grass.osgeo.org/grass70/manuals/addons/r.geomorphon.html}}} % image source
...
%The mean elevation difference -- i.e.~
...
%I start with the ridges and high points. Then 
...
Finding the relative height, however, is challenging -- it is subtle.'
...
% describe the need for serious games
...
% Setting up the experiment % Experimental procedure
...
%\section{Review of tangible interfaces for geospatial modeling}\label{appendix:review}
...
%&& Gesture &&& \cite{Blackshaw2011}\\
%%
%Collaborative Design Platform & Augmented architectural models & Object detection &&& \cite{Schubert2011a}\\
%&& Touch &&& \cite{Schubert2012}\\
%&& Sketching &&&  \cite{Schubert2014}\\
%&&&&& \cite{Schubert2015}\\
%%&&&&& cite{Schubert2011}\\\
